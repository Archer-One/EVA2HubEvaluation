...
//        System.load("/home/archer-oneee/Desktop/MyWork/bn128-jni/native/target/release/libbn_jni.so");
        System.load("/home/archer-oneee/Desktop/MyWork/native-libs/libbn_jni.so");
    }
    ...
    public static byte[] HashToG1(BigInteger num) throws Exception {
        byte[] num_data = num.toByteArray();
        assert (num_data.length <= WORD_SIZE);

        byte[] num_aligned = new byte[WORD_SIZE];
        System.arraycopy(num_data, 0, num_aligned, WORD_SIZE - num_data.length, num_data.length);

        return Holder.INSTANCE.HashToG1(num_aligned);
    }
